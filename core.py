...
from database import (
    ...
    sync_all_files, sync_phrases_to_file, sync_ranks_to_file  # Добавлено
)
...
DB_PATH = "dori.db"  # Путь к базе данных
...
async def process_shop(callback: CallbackQuery):
    ...
    if callback.data == "buy_shield":
        ...
        # Проверяем, есть ли уже активный щит
        if user['shield_until']:
            try:
                ...
                pass  # Если дата невалидная, игнорируем
...
# ============================================================
# 5. АДМИНСКОЕ МЕНЮ /sms (с пагинацией по 15 и зацикливанием)
# ============================================================
...
async def cmd_sms(message: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔄 Синхронизировать файлы", callback_data="sync_files")]  # Новая кнопка
    ])
    await message.answer("📋 Панель управления контентом:", reply_markup=kb)

async def admin_images_menu(callback: CallbackQuery):
    await callback.message.edit_text("🖼️ Раздел 'Картинки без текста' в разработке.")

async def admin_memes_menu(callback: CallbackQuery):
    await callback.message.edit_text("📝🖼️ Раздел 'Картинки с текстом' в разработке.")

# ===== ПАГИНАЦИЯ С ЗАЦИКЛИВАНИЕМ =====

async def admin_phrases_list(callback: CallbackQuery):
    """Показывает первую страницу фраз (по 15 штук)"""
    ...
    max_page = max(1, (total + 14) // 15)  # Округление вверх
...
async def next_page(callback: CallbackQuery):
    ...
    if next_page_num > max_page:
        next_page_num = 1  # Зацикливание: с последней на первую
...
async def prev_page(callback: CallbackQuery):
    ...
    if prev_page_num < 1:
        prev_page_num = max_page  # Зацикливание: с первой на последнюю
...
async def process_remove_number(message: Message, state: FSMContext):
    ...
    # Синхронизируем файл после удаления
...
async def show_rank_phrases(callback: CallbackQuery):
    ...
    phrases = await get_phrases_by_trigger("RANK", limit=999, mood=rank_name)  # limit=999 для всех
...
# ============================================================
# 7. ОБРАБОТКА ВСЕХ СООБЩЕНИЙ (начисление коинов и стрика)
# ============================================================
...
async def process_message(message: Message):
    ...
    # Проверяем, не пора ли повысить ранг
...
# ============================================================
# 8. FSM ШАГИ (конструктор с одной строкой: текст | эмодзи)
# ============================================================
...
async def save_phrase(callback: CallbackQuery, state: FSMContext):
    ...
    # Синхронизируем файл после добавления
    await sync_phrases_to_file()
...
# ============================================================
# 9. ПЛАНИРОВЩИК (обрабатывает и 1 день, и 2+ дня)
# ============================================================
