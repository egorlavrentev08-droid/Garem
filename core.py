...
from database import (
    ...
    get_user_by_identifier,  # ⚠️ ЗАМЕНИЛИ get_user_by_name
...
async def cmd_coins(message: Message):
    ...
    action = args[1]
    identifier = args[2].replace('@', '')  # ⚠️ убираем @
    try:
        amount = float(args[3])
    except ValueError:
        return await message.answer("❌ Сумма должна быть числом.")
    
    # ⚠️ Используем новую функцию поиска
...
async def cmd_bypass(message: Message):
    ...
    identifier = args[1].replace('@', '')  # ⚠️ убираем @
    try:
        hours = int(args[2])
    except ValueError:
