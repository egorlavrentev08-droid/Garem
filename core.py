...
# ============================================================
# 7. ПЛАНИРОВЩИК (ТРИ ИСХОДА)
# ============================================================

async def check_inactive_users(bot: Bot):
    """Проверяет бездействие и отправляет: фразу+пикчу, мем или ранг+рангпикчу"""
    ...
    for user in inactive:
        ...
        # Рандомный выбор исхода (1 из 3)
        outcome = random.choice(['phrase_pic', 'meme', 'rank_phrase_pic'])
        
        try:
            if outcome == 'phrase_pic':
                # 1) Фраза + картинка из pic/
                pic_folder = 'Content/pic/'
                if os.path.exists(pic_folder) and os.listdir(pic_folder):
                    pic_file = random.choice(os.listdir(pic_folder))
                    pic_path = os.path.join(pic_folder, pic_file)
                    await bot.send_photo(user['user_id'], photo=FSInputFile(pic_path), caption=text)
                else:
                    await bot.send_message(user['user_id'], text)
            
            elif outcome == 'meme':
                # 2) Только мем из mems/
            ...
            elif outcome == 'rank_phrase_pic':
                # 3) Фраза ранга + картинка из rankpic/
                # Сначала получаем ранг пользователя (берём из БД)
                user_data = await get_user(user['user_id'])
                rank = user_data['rank'] if user_data else 'Новичок'
                
                # В будущем тут можно брать фразу из ranksms.txt по рангу
                rank_text = f"🏆 Поздравляю с новым рангом: {rank}!"
                
                rankpic_folder = 'Content/rankpic/'
                if os.path.exists(rankpic_folder) and os.listdir(rankpic_folder):
                    rank_file = random.choice(os.listdir(rankpic_folder))
                    rank_path = os.path.join(rankpic_folder, rank_file)
                    await bot.send_photo(user['user_id'], photo=FSInputFile(rank_path), caption=rank_text)
                else:
                    await bot.send_message(user['user_id'], rank_text)
        
        except Exception as e:
            # Если что-то пошло не так — отправляем только текст
            await bot.send_message(user['user_id'], text)
