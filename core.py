...
from database import (
    ...
    add_reward_history,
    add_shield, use_shield, get_shield_count  # <-- НОВЫЕ ФУНКЦИИ
)

# Импорт из cofe.py (чат-библиотека)
from cofe import (
    phrase_cache, load_phrases_from_chat,
    get_random_phrase, get_rank_phrase_from_cache,
...
# ============================================================
# FSM СОСТОЯНИЯ (только для магазина)
# ============================================================
...
async def process_shop(callback: CallbackQuery):
    ...
    if callback.data == "buy_shield":
        ...
        # Списываем коины и добавляем щит
...
async def process_message(message: Message):
    ...
    if redemption and redemption['active']:
        ...
        if progress >= target:
            ...
            await message.answer(
                ...
                f"Написано {target} сообщений за сутки! 🦊"
            )
            ...
            if progress % 50 == 0:
                remaining = target - progress
                await message.answer(
                    f"📊 {progress}/{target} сообщений до восстановления. "
    ...
    for min_streak, rank_name in sorted(rank_mapping.items()):
        if current_streak >= min_streak and user['rank'] != rank_name:
            ...
            rank_phrase = await get_rank_phrase_from_cache(rank_name)
            if rank_phrase:
    ...
    if current_streak in achievement_days:
        ...
        if not already:
            phrase = await get_streak_achievement_from_cache(current_streak)
            if not phrase:
...
async def check_inactive_users(bot: Bot):
    from database import get_inactive_users, set_shield, update_streak, start_redemption
        ...
        if 24 <= hours_since < 48:
            ...
            if shield_active:
                ...
                await bot.send_message(
                    ...
                    f"🔄 **Шанс восстановить!**\n"
                    f"Напиши **200 сообщений** за сегодня — и стрик вернётся!\n\n"
