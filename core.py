...
def register_handlers(dp):
    ...
    dp.message.register(cmd_redemption_stats, Command("redemption_stats"), chat_filter)
...
async def cmd_top_messages(message: Message):
    """Показывает топ-15 по сообщениям за сегодня"""
...
async def cmd_redemption(message: Message):
    """Показывает текущий прогресс искупления"""
    user = await get_user(message.from_user.id)
    if not user:
        return await message.answer("Сначала зарегистрируйся: /start")
...
async def cmd_redemption_stats(message: Message):
    if not is_admin(message.from_user.id):
        return await message.answer("❌ Ты не админ.")
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute("""
            SELECT 
                ...
                SUM(CASE WHEN redemption_active = 1 THEN 1 ELSE 0 END) as active,
                SUM(CASE WHEN redemption_active = 0 AND redemption_progress >= redemption_target THEN 1 ELSE 0 END) as completed,
                AVG(redemption_progress) as avg_progress
            FROM users
            WHERE redemption_streak_to_restore > 0 OR redemption_active = 1
        """)
        stats = await cursor.fetchone()
        
        # Топ успешных искуплений
        cursor2 = await db.execute("""
            SELECT name, redemption_streak_to_restore, redemption_progress
            FROM users
            WHERE redemption_active = 0 
              AND redemption_progress >= redemption_target
              AND redemption_streak_to_restore > 0
            ORDER BY redemption_streak_to_restore DESC
            LIMIT 5
        """)
        top_redemptions = await cursor2.fetchall()
    
    text = f"📊 **Статистика искуплений**\n\n"
    text += f"Всего попыток: {stats[0] or 0}\n"
    text += f"Активных: {stats[1] or 0}\n"
    text += f"Успешных: {stats[2] or 0}\n"
    text += f"Процент успеха: {round(stats[2] / stats[0] * 100 if stats[0] and stats[0] > 0 else 0, 1)}%\n"
    text += f"Средний прогресс: {round(stats[3] or 0, 1)} сообщений\n\n"
    
    if top_redemptions:
        text += "🏆 **Лучшие восстановления:**\n"
        for name, streak, progress in top_redemptions[:5]:
            text += f"• {name or 'Без имени'} — {streak} дней ({progress} сообщений)\n"
    
    await message.answer(text)

# ============================================================
# 7. АДМИНСКОЕ МЕНЮ /sms (полное)
# ============================================================
...
# ============================================================
# 8. УПРАВЛЕНИЕ ФРАЗАМИ (с выбором триггера)
# ============================================================

async def admin_phrases_list(callback: CallbackQuery, state: FSMContext):
    """Показывает меню выбора триггера"""
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def choose_trigger(callback: CallbackQuery, state: FSMContext):
    """Обрабатывает выбор триггера и показывает фразы"""
    trigger_map = {
...
# ============================================================
# 11. УДАЛЕНИЕ ФАЙЛОВ (общее для pic/ и mems/)
# ============================================================
...
# ============================================================
# 13. ОБРАБОТКА ВСЕХ СООБЩЕНИЙ (ОСНОВНАЯ ЛОГИКА)
# ============================================================
...
# ============================================================
# 14. ПЛАНИРОВЩИК - ПРОВЕРКА НЕАКТИВНЫХ
# ============================================================
    ...
    for user in inactive:
        ...
        if 24 <= hours_since < 48:
            ...
            if shield_active:
                ...
                # Отправляем мем или фразу для усиления эффекта
...
# ============================================================
# 15. ПЛАНИРОВЩИК - ЕЖЕДНЕВНЫЕ И ЕЖЕНЕДЕЛЬНЫЕ НАГРАДЫ
# ============================================================

async def daily_reset_and_reward(bot: Bot):
    """Сбрасывает счётчик и награждает первого места"""
...
async def check_expired_redemptions_task(bot: Bot):
    """Проверяет просроченные искупления и уведомляет"""
