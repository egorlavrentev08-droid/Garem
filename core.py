...
def register_handlers(dp):
    ...
    # 🔥 ВАЖНО: Добавляем F.chat.id == CHAT_ID, чтобы не перехватывать сообщения из чата-библиотеки
    dp.message.register(process_message, F.chat.id == CHAT_ID, chat_filter)
...
async def process_message(message: Message):
    # Эта функция вызывается ТОЛЬКО для сообщений из основного чата (CHAT_ID)
    # Благодаря фильтру F.chat.id == CHAT_ID в register_handlers
