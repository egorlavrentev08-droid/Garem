...
# ============================================================
# 0. ФИЛЬТР ПРОВЕРКИ УЧАСТНИКА ЧАТА (САМОЕ ВАЖНОЕ)
# ============================================================
...
def register_handlers(dp):
    """Регистрирует все хендлеры с фильтром чата"""
    
    # Регистрируем команды с фильтром (автоматически применяется ко всем)
    ...
    # Кнопки меню админа (не требуют фильтра, так как вызываются из чата)
...
async def cmd_coins(message: Message, command: CommandObject):
    ...
    # В реальности нужно дописать get_user_by_name() в database.py
