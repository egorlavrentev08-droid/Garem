...
async def main():
    ...
    # ============================================================
    # 🔥 УНИВЕРСАЛЬНЫЙ ЛОГГЕР ВСЕХ СООБЩЕНИЙ (САМЫЙ ПЕРВЫЙ)
    # ============================================================
    
    @dp.message()
    async def log_all_messages(message: Message):
        """Логирует все сообщения, которые получает бот"""
        logger.info(f"📩 ПОЛУЧЕНО СООБЩЕНИЕ: от {message.from_user.id} (@{message.from_user.username}) в чат {message.chat.id} ({message.chat.type}): {message.text}")
    
    logger.info("✅ Универсальный логгер зарегистрирован")
    
    # ============================================================
    # 🔥 КОМАНДЫ ДЛЯ КЕША (СРАЗУ ПОСЛЕ ЛОГГЕРА, ЧТОБЫ ТОЧНО РАБОТАЛИ)
    # ============================================================
    ...
    async def cmd_update(message: Message):
        ...
        await message.answer("🔄 Обновляю кеш из чата-библиотеки...")
