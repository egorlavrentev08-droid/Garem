...
async def on_startup(bot: Bot):
    ...
    logger.info("📚 Загрузка фраз из кеша...")
    total = load_phrases_from_chat()  # ← Теперь без аргументов!
...
async def main():
    ...
    # 2. Админские команды общего назначения
    ...
    # ============================================================
    # 🔥 КОМАНДЫ ДЛЯ КЕША (С ФИЛЬТРОМ chat_filter)
    # ============================================================
    
    @dp.message(Command("check"), chat_filter)
    ...
    @dp.message(F.text.lower() == ".чек", chat_filter)
    ...
    @dp.message(Command("upd"), chat_filter)
    async def cmd_update(message: Message):
        logger.info(f"🔍 КОМАНДА /upd от {message.from_user.id}")
        if not is_admin(message.from_user.id):
            return await message.answer("❌ Только админы!")
        
        await message.answer("🔄 Обновляю кеш из файла...")
        total = load_phrases_from_chat()  # ← Теперь без аргументов!
    ...
    @dp.message(F.text.lower() == ".апд", chat_filter)
