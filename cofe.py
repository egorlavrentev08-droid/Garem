...
# ============================================================
# ЗАГРУЗКА ФРАЗ ИЗ ЧАТА ПРИ СТАРТЕ
# ============================================================
...
async def library_parser(message: Message):
    ...
    if len(clean_text) < 7:
        reply = await message.reply(
            f"❌ Слишком коротко! Минимум: 7 символов\nПример: `!₽ Где ты был?`"
        )
        asyncio.create_task(delete_message_after_delay(message.bot, message.chat.id, reply.message_id, 60))
        return
    ...
    if len(clean_text) > 100:
        reply = await message.reply(
            f"❌ Слишком длинно! Максимум: 100 символов"
        )
        asyncio.create_task(delete_message_after_delay(message.bot, message.chat.id, reply.message_id, 60))
        return
    ...
    if trigger_symbol not in TRIGGER_SYMBOLS:
        ...
        reply = await message.reply(f"❌ Неверный триггер! Доступны: {available}")
        asyncio.create_task(delete_message_after_delay(message.bot, message.chat.id, reply.message_id, 60))
        return
    
    if mood_symbol not in MOOD_SYMBOLS:
        available = ', '.join(MOOD_SYMBOLS.keys())
        reply = await message.reply(f"❌ Неверное настроение! Доступны: {available}")
        asyncio.create_task(delete_message_after_delay(message.bot, message.chat.id, reply.message_id, 60))
        return
