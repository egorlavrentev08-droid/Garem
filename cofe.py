...
from database import save_phrases_to_db, load_phrases_from_db, delete_all_phrases_from_db
...
async def sync_cache_to_db():
    """Сохраняет текущий кеш в БД"""
    for trigger, phrases in phrase_cache.items():
        await save_phrases_to_db(trigger, phrases)
    logger.info("💾 Кеш сохранён в БД")
