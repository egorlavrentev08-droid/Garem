...
def register_library_handlers(dp: Dispatcher):
    # ВРЕМЕННЫЙ ЛОГГЕР ДЛЯ ДИАГНОСТИКИ (ОТКЛЮЧАЕМ, ЧТОБЫ НЕ БЛОКИРОВАЛ ПАРСЕР)
    # dp.message.register(debug_library, F.chat.id == LIBRARY_CHAT_ID)
...
# ============================================================
# ВРЕМЕННЫЙ ЛОГГЕР ДЛЯ ДИАГНОСТИКИ (ОСТАВЛЯЕМ, НО НЕ РЕГИСТРИРУЕМ)
# ============================================================

async def debug_library(message: Message):
    """Логирует все сообщения в чате-библиотеке"""
    logger.info(f"🔍 СООБЩЕНИЕ В БИБЛИОТЕКЕ: {message.text} от {message.from_user.id}")
    await message.reply(f"✅ Бот видит сообщение: {message.text[:50] if message.text else 'Нет текста'}...")
...
async def load_phrases_from_chat(bot):
    ...
    logger.info(f"📂 Файл {CACHE_FILE} не найден, создаём новый")
    save_cache_to_file()
    return 0
