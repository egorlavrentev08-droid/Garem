...
async def load_phrases_from_chat(bot: Bot):
    ...
    try:
        ...
        # Пытаемся использовать get_chat_history (если есть)
        try:
            # В aiogram 3.x может быть get_chat_history
            async for message in bot.get_chat_history(LIBRARY_CHAT_ID, limit=max_messages):
                all_messages.append(message)
                total_loaded += 1
                if total_loaded >= max_messages:
                    break
            logger.info(f"📥 Загружено {total_loaded} сообщений через get_chat_history")
        except AttributeError:
            # Если get_chat_history нет, пробуем get_chat_messages с пагинацией
            logger.info("📥 get_chat_history недоступен, пробуем get_chat_messages...")
        ...
        # Если сообщений нет - пробуем получить через get_chat
        if not all_messages:
            try:
                # Получаем последние сообщения через get_chat
                chat = await bot.get_chat(LIBRARY_CHAT_ID)
                logger.info(f"📥 Чат найден: {chat.title}")
                
                # Пробуем получить сообщения через get_chat_messages с конкретными ID
                # Получаем последние 100 ID
                for i in range(1, 101):
                    try:
                        msg = await bot.get_chat_messages(LIBRARY_CHAT_ID, message_ids=i)
                        if msg:
                            all_messages.append(msg)
                    except:
                        pass
                logger.info(f"📥 Загружено {len(all_messages)} сообщений через перебор ID")
            except Exception as e:
                logger.warning(f"⚠️ Ошибка при получении чата: {e}")
